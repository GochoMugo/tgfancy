...
exports = module.exports = class Tgfancy extends TelegramBot {
    ...
    constructor(token, options={}) {
        ...
        if (opts.openshiftWebHook && process.env.OPENSHIFT_APP_UUID) {
            ...
            // since we are using a web hook, we do not need polling
            delete opts.polling;
        }
        ...
        if (this.options._openshift && process.env.OPENSHIFT_APP_UUID) {
            this.setWebHook(this.options._openshiftDomain);

        // setting up listening to updates via websocket
        } else if (this.options.webSocket) {
            if (options.polling || options.webHook) {
                throw new Error("Refusing to try to listen via WebSocket"
                    + " -- polling or WebHook already set");
            }
            this._WebSocket = TelegramWebSocket(
                token,
                this.options.webSocket,
                _.bindKey(this, "processUpdate")
            );
        }
    }
            ...
            function exec(resolve, reject) {
                method.call(self, ...args)
                    ...
                    .catch(function(error) {
                        ...
                        const opts = self.options.ratelimitingOptions;
                        retry++;
    ...
    _emojify(method, methodDesc) {
        ...
        return function() {
            ...
            const emojifiedText = self.options.emojify(text);
