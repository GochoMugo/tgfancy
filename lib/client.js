 ...
 * 4. Before Plank was available, we relied on the bridge run by @GingerPlusPlus
 */
...
const plank = require("plank-client");
...
class Tgfancy extends TelegramBot {
            ...
            function exec(resolve, reject) {
                method.call(self, ...args)
                    ...
                    .catch(function(error) {
                        // TODO: improve detecting 429 errors,
    ...
    _resolveChatId(method, options={}) {
        ...
        return function() {
            ...
            return Promise.mapSeries(usernames, function(username) {
                ...
                return self.tgfancy.chatIdResolution.resolve(self.token, username)
    ...
    openWebSocket() {
        ...
        this._ws = new plank.WebSocket(this.tgfancy.token);
        debug("set up websocket updates: %s", this._ws.urls.websocket);
        this._ws.on("message", (payload) => {
            // NOTE: this is almost as replicating how the request listener in
            // the webhook client/component in NTBA.
            if (payload.method !== "POST") return;
            this.processUpdate(payload.body);
        });
        return new Promise((resolve, reject) => {
            this._ws.on("ready", () => {
                return this.setWebHook(this._ws.urls.websocket.replace(/^ws/, "http"))
                    .resolve(resolve)
                    .reject(reject);
            });
        });
